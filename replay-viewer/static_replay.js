// Escrow static replay shell: fetches the replay named by ?replay=<url>,
...
//
// The wasm module is built with -s MODULARIZE=1 -s
// EXPORT_NAME=EscrowReplayModule (replay-viewer/config.nims), so it is a
// FACTORY: this shell calls EscrowReplayModule() and awaits the promise.
// Waiting on Module.onRuntimeInitialized instead would hang forever with
// every file present and every asset 200 (cogame-lantern, 2026-08-23) --
// the flags and the bootstrap are a matched pair, never mix lineages.
(function () {
  "use strict";
  ...
  function start(module, bytes) {
    ...
    var ok = module._esc_load_replay(ptr, bytes.length);
    module._free(ptr);
    if (!ok) {
      fail(readString(module, module._esc_error_ptr(),
        module._esc_error_len()) || "wasm rejected the replay");
      return;
    }
    var payload = JSON.parse(
      readString(module, module._esc_payload_ptr(),
        module._esc_payload_len())
    );
    var loading = document.getElementById("loading");
    if (loading) loading.style.display = "none";
    document.documentElement.removeAttribute("data-replay-error");
    EscrowRenderer.attachReplay({
    ...
    // later so "ready" means a picture, not merely a parsed payload. The
    // attribute and the bridge message go together: tools/ci/viewer_smoke.mjs
    // waits for either, and a top-level load has no parent to postMessage to.
  ...
  function load() {
    ...
    caption(attempt > 1 ? "RETRYING REPLAY… (attempt " + attempt + ")" :
      "LOADING REPLAY…");
    if (!modulePromise) {
      modulePromise = EscrowReplayModule().catch(function (error) {
