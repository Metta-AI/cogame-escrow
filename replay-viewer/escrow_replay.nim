## Escrow static replay viewer, wasm side.
##
## JS hands the raw replay bytes to esc_load_replay; this module parses
## them with the SAME sim code the game server runs, re-derives the
## per-event floor states, and exposes the enriched payload (identical
...
proc escLoadReplay(data: ptr uint8, length: cint): cint
    {.exportc: "esc_load_replay", cdecl.} =
  try:
    lastError = ""
    let replay = parseJson(bytesFromPointer(data, int(length)))
    var config = defaultGameConfig()
    config.turns = replay["config"]{"turns"}.getInt(16)
    ...
    payload = $ %*{
      "type": "replay",
      "protocol": replay{"protocol"}.getStr("escrow.replay.v1"),
      "names": replay["names"],
...
proc escPayloadPointer(): ptr uint8 {.exportc: "esc_payload_ptr", cdecl.} =
  if payload.len == 0:
    nil
  else:
    cast[ptr uint8](payload[0].addr)

proc escPayloadLength(): cint {.exportc: "esc_payload_len", cdecl.} =
  cint(payload.len)

proc escErrorPointer(): ptr uint8 {.exportc: "esc_error_ptr", cdecl.} =
  if lastError.len == 0:
    nil
  else:
    cast[ptr uint8](lastError[0].addr)

proc escErrorLength(): cint {.exportc: "esc_error_len", cdecl.} =
  cint(lastError.len)
