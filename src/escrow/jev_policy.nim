...
proc chooseAction*(observation: JsonNode): JsonNode =
  ...
  for contract in observation["board"]:
    ...
    for good, amount in contract["ask"].pairs:
      if amount.getInt() > stock[good].getInt():
        affordable = false
    if affordable:
      actions["sign_" & contract["id"].getStr()] =
        %*{"sign": [contract["id"]]}
  ...
  let body = %*{
    "model": model,
    "state": "You are playing Escrow. Maximize your final hearts from " &
      "commission fills. This seat-private observation is all you may use:\n" &
      $observation,
    "questions": {"decision": {
      "type": "choice",
