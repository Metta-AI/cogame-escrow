## Escrow game server: implements the Coworld game contract.
##
## Endpoints:
...
proc policyNamesJson(gs: GameState): JsonNode =
  ## Seats trade under anonymous cog names; the policy names ride alongside
...
proc broadcastLocked(gs: GameState) =
  ## Callers hold stateLock. Spectators get the whole floor; players get
  ## their own booth plus the public board.
...
proc replayPayload(gs: GameState, results: JsonNode): string =
  ...
  for event in gs.sim.events:
    events.add(event.eventToJson())
  $ %*{
    "protocol": "escrow.replay.v" & $ReplayVersion,
    "names": names,
...
proc finishEpisode(runtimeConfig: RuntimeConfig) =
  ...
  sleep(500)
  echo "escrow: writing results and replay"
  writeArtifact(
  ...
  sleep(500)
  echo "escrow: episode complete, shutting down"
  quit(0)
...
proc runGame(runtimeConfig: RuntimeConfig) {.gcsafe.} =
  {.gcsafe.}:
    ...
    withLock stateLock:
      state.started = true
      echo "escrow: starting with ", state.playerSockets.len, "/",
    ...
    ## Attempt 0 + one retry batch + applying the turn. Every wait in the
    ## turn is bounded by this, so the pre-turn check below can promise
    ## that starting a turn cannot overrun the play budget.
    let maxTurnSeconds = 2.0 * config.llmTimeoutSeconds.float + 5.0
    if playDeadline > 0.0:
      echo "escrow: episode timeout ", timeoutSeconds.int, "s (",
        (if hostedTimeout.len > 0: "from env" else: "assumed"),
        "); playing until ", (timeoutSeconds * PlayBudgetFraction).int,
        "s, at most ", maxTurnSeconds.int, "s per turn"
    ...
    while true:
      ...
      withLock stateLock:
        ...
        if playDeadline > 0.0 and
            epochTime() + maxTurnSeconds > playDeadline:
          ## The platform kills an episode that outruns its timeout and
          ## keeps nothing at all, so give up turns rather than the whole
          ## result: stop here, between turns, with the horizon closure
          ## run so no escrow is stranded.
          echo "escrow: episode deadline reached after ",
      ...
      withLock stateLock:
        for index, seat in seats:
          ...
          echo "escrow: turn ", state.sim.turn, " ", state.sim.names[seat],
            ...
            (if decision.offer.len > 0: " offers" else: ""),
          ...
          try:
            ...
            echo "escrow: reply rejected (", error.msg,
              "); using the trader baseline"
            let fallback = scriptedAction(state.sim, seat, skTrader)
            state.sim.applyMove(seat, fallback, true)
        state.broadcastLocked()

      ## Pace between turns so spectators can read the floor.
      if config.turnDelayMs > 0:
        sleep(config.turnDelayMs)

    ## Let the last turn land before the final frame.
    if config.turnDelayMs > 0:
      sleep(config.turnDelayMs)
...
proc playerUpgradeHandler(request: Request) {.gcsafe.} =
  {.gcsafe.}:
    ...
    withLock stateLock:
      ...
      echo "escrow: player slot ", slot, " connected (",
      ...
      try:
        ...
        if payload{"type"}.getStr() == "prompt":
          ...
          let scripted =
            ...
            elif node.kind == JBool: (if node.getBool(): skTrader
              else: skNone)
        ...
        echo "escrow: ignoring bad player frame: ", error.msg
    of ErrorEvent:
      discard
    of CloseEvent:
      withLock stateLock:
...
proc configFromReplay*(payload: JsonNode): GameConfig =
  ...
  result.turns = payload["config"]{"turns"}.getInt(16)
  result.seed = payload["config"]{"seed"}.getInt(0)
  result.talk = payload["config"]{"talk"}.getBool(true)
  ## The replay carries the episode's fitted cap; never re-fit it. Profiles
  ## and aliases are re-derived from the seed.
  result.sampled = true
...
proc runReplayServer*(runtimeConfig: RuntimeConfig) =
  ...
  var enriched = %*{
    "type": "replay",
    "protocol": payload{"protocol"}.getStr("escrow.replay.v1"),
    "names": payload["names"],
  ...
  echo "escrow: replay mode on ", runtimeConfig.host, ":", runtimeConfig.port
  gameServer.serve(Port(runtimeConfig.port), runtimeConfig.host)

proc runGameServer*(config: GameConfig, runtimeConfig: RuntimeConfig) =
  if config.tokens.len != config.players.len:
    raise newException(EscrowError, "tokens and players must align")
  state.config = config
  ...
  echo "escrow: serving on ", runtimeConfig.host, ":", runtimeConfig.port
  gameServer.serve(Port(runtimeConfig.port), runtimeConfig.host)
