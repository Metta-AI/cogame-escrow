## Server-side decisions for Escrow. Prompt policies ask Claude for gives,
## offers, signatures, and speech. Jev policies rank validated actions
## generated from the trader baseline.
##
...
## With no usable model transport a seat falls back to the always-legal
...
proc newLlmClient*(config: GameConfig): LlmClient =
  ...
  if bedrockEndpoint.len > 0:
    ...
    if result.jevKey.len == 0:
      raise newException(EscrowError, "METTA_CAPTURE_KEY is required")
    result.jevModel = "typesafe/jev-1.13"
    result.jevTrajectoryId = "escrow-jev-" & $config.seed
  ...
  if result.apiKey.len > 0:
    ...
    if result.jevEndpoint.len > 0:
      result.curl = newCurly()
    echo "escrow llm: no Claude credentials; prompt seats use scripted fallback"
...
type JevChoices* = object
  criteria*: JsonNode
  moves*: seq[Decision]

proc jevChoices*(sim: Sim, seat: int): JevChoices =
  ## Present actions generated by the legal trader baseline, including
  ## smaller and larger contracts, signing alone, and passing.
  result.criteria = newJObject()
  let trader = scriptedAction(sim, seat, skTrader)
  var candidates = @[trader, Decision()]
  for units in [2, 4, 8, 12]:
    var params = DefaultTraderParams
    params.tradeUnits = units
    candidates.add(traderAction(sim, seat, params))
  for fills in [1, 5]:
    var params = DefaultTraderParams
    params.needFills = fills
    candidates.add(traderAction(sim, seat, params))
  if trader.signs.len > 0:
    var signOnly = trader
    signOnly.offer = ""
    candidates.add(signOnly)
  if trader.offer.len > 0:
    var offerOnly = trader
    offerOnly.signs = @[]
    candidates.add(offerOnly)
  for candidate in candidates:
    if sim.validateMove(seat, candidate).len > 0 or
        candidate in result.moves:
      continue
    let key = $result.moves.len
    result.moves.add(candidate)
    var description = "Pass this turn"
    if candidate.offer.len > 0:
      description = "Post this contract:\n" & candidate.offer
    if candidate.signs.len > 0:
      description.add("\nSign contracts: " & candidate.signs.join(", "))
    result.criteria[key] = %description

proc jevDecision*(payload: JsonNode, choices: JevChoices): Decision =
  let answer = payload["answers"]["decision"]
  let probabilities = answer["probabilities"]
  let reported = answer["choice"].getStr()
  if answer["type"].getStr() != "choice" or
      not choices.criteria.hasKey(reported) or
      probabilities.len != choices.moves.len:
    raise newException(EscrowError, "Jev returned the wrong choice set")
  let confidence = answer["confidence"].getFloat()
  if confidence < 0 or confidence > 1:
    raise newException(EscrowError, "Jev confidence is outside [0, 1]")
  var total = 0.0
  var best = -1.0
  var choice = ""
  for name, probability in probabilities.pairs:
    if not choices.criteria.hasKey(name):
      raise newException(EscrowError, "Jev returned an unknown choice")
    let value = probability.getFloat()
    if value < 0 or value > 1:
      raise newException(EscrowError, "Jev probability is outside [0, 1]")
    total += value
    if value > best:
      best = value
      choice = name
  if abs(total - 1) > probabilities.len.float * 0.005 + 1e-6:
    raise newException(EscrowError, "Jev probabilities do not sum to one")
  result = choices.moves[parseInt(choice)]
  echo "escrow jev: choice ", choice, " reported ", reported,
...
proc decideAll*(
  ...
  for attempt in 0 .. 1:
    if client.disabled:
      ...
      for index in open:
        if jev[seats[index]]:
          ...
          result[index] = SeatDecision(
            move: scriptedAction(sim, seats[index], skTrader),
            scripted: true)
      open = enabled
    if open.len == 0:
      break
    ...
    var choices = newSeq[JevChoices](seats.len)
    for index in open:
      let seat = seats[index]
      if jev[seat]:
        choices[index] = sim.jevChoices(seat)
        var headers: HttpHeaders
        headers["content-type"] = "application/json"
        ...
        let body = %*{
          ...
          "questions": {"decision": {
            "type": "choice",
            "instructions": "Choose the action that maximizes final hearts from commission fills. A fair six-unit swap can fill more commissions than a two- or four-unit swap; passing can forfeit a turn of production. Prefer a feasible fair offer or valuable signature unless the visible board gives a concrete reason not to trade.",
            "criteria": choices[index].criteria
          }}
        }
      ...
      try:
        ...
        if jev[seat]:
          ...
          if error.len > 0 or response.code < 200 or response.code >= 300:
            raise newException(EscrowError, "Jev transport failed: " &
              error & " HTTP " & $response.code)
          decision = jevDecision(parseJson(response.body), choices[index])
        else:
          let text = client.textOf(responses[position].response,
            responses[position].error, batch[position].url)
          decision = parseDecision(extractJsonObject(text), sim, seat)
