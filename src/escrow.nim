## Escrow entrypoint: reads the Coworld runtime contract and starts either
...
proc randomSeed(): int =
  ...
  if not urandom(buf):
    raise newException(EscrowError, "OS entropy source unavailable")
...
when isMainModule:
  ...
  if runtimeConfig.replayMode:
    ...
    if not seedPinned(runtimeConfig.config):
      ## An unpinned seed is randomized so the profile deal and the
      ## aliases are not precomputable.
      config.seed = randomSeed()
      echo "escrow: seed not pinned; randomized"
